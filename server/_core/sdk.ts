...
import { ENV } from "./env";
import { buildLocalAuthenticatedUser } from "./localAuth";
import type {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (session.openId.startsWith("manager:")) {
      ...
      if (
        ...
        throw ForbiddenError("Manager session user not found");
      }
      await db.upsertUser({ openId: manager.openId, lastSignedIn: new Date() });
      return { ...manager, lastSignedIn: new Date() } as AuthenticatedUser;
    }
